...
public class ProposalApproveAfterEndService implements AfterEndService
{
    ...
    private String mAllPolPassFlag = "9"; //所有险种通过标记
        ...
        for (nPolIndex = 1; nPolIndex <= nPolCount; nPolIndex++)
        {
            ...
            else //目前目前所有的险种均有一些公共的核保规则,所以必定走该分枝
            {
                ...
                for (int i = 1; i <= n; i++)
                {
                    ...
                    if (CheckPol(tLCPolSchema.getInsuredNo(),
                                 tLCPolSchema.getRiskCode()) == 0)
                    {
                    }
                    else
                    {
...
//                        + "级别:" + mUWGrade);
            ...
            //只要有一个险种未通过则将所有险种通过标识置为未通过 Alex 20050925
        ...
        //如果合同级自核通过但险种级自核不过时，将合同自核通过标识仍然置为不通过 Alex 20050925
        if ("9".equals(mContPassFlag) && "5".equals(mAllPolPassFlag))
        {
            mContPassFlag = "5";
        }
...
//            return false;
    ...
    private boolean prepareTransferData()
    {
        ...
        //由于在前面已经判断了合同级和险种级自核的关系，所以这里就不判断 Alex 20050925
