...
public class UWAutoAgentAfterInitService implements AfterInitService
{
...
//    private GlobalInput mGlobalInput = new GlobalInput();
    ...
    public UWAutoAgentAfterInitService()
    {
    }
    ...
    {
        UWPrepareAgentBL tUWPrepareAgentBL = new UWPrepareAgentBL();
        if (!tUWPrepareAgentBL.submitData(mInputData, ""))
        {
            CError tError = new CError();
            tError.moduleName = "UWPrepareIssueBL";
            tError.functionName = "delaData";
            tError.errorMessage = "向UWPrepareIssueBL传输数据出错!";
            this.mErrors.checkErrors(tUWPrepareAgentBL.mErrors);
            return false;
        }
        try
        {
            mLOPRTManagerSchema = (LOPRTManagerSchema) tUWPrepareAgentBL.getResult().
                    get(0);
            tLCIssuePolSet = (LCIssuePolSet) tUWPrepareAgentBL.getResult().get(1);
        }
        catch (Exception e)
        {
            e.printStackTrace();
...
//工作流，置为函件未回收
    ...
    private boolean prepareTransferData()
    {
        ...
        mTransferData.setNameAndValue("PrtSeqOper", mLOPRTManagerSchema.getPrtSeq());
        mTransferData.setNameAndValue("PrtSeqOper",
