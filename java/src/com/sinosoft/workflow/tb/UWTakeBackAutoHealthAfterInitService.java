...
public class UWTakeBackAutoHealthAfterInitService implements AfterInitService
{
...
//    private LCCUWMasterSchema mLCCUWMasterSchema = new LCCUWMasterSchema();
    ...
    public UWTakeBackAutoHealthAfterInitService()
    {
    }
...
//        mLCCUWMasterSchema.setSchema(tLCCUWMasterDB);
        ...
        for (int i = 1; i <= mLOPRTManagerSet.size(); i++)
        {
                    ...
                    mLOPRTManagerSet.get(i).getStateFlag().trim().equals("1"))
            {
                ...
                tLZSysCertifyDB.setCertifyCode(PrintManagerBL.CERT_CODE_PE); //体检通知书标识
                ...
                if (tLZSysCertifySet == null || tLZSysCertifySet.size() != 1)
                {
                    ...
                    CError tError = new CError();
                    tError.moduleName =
                            "UWPrintTakeBackAutoHealthAfterInitService";
                    tError.functionName = "preparePrint";
...
//        mLCCUWMasterSchema.setHealthFlag("3"); //发送核保通知书
        ...
        tLCPENoticeDB.setProposalContNo(mContNo);
        tLCPENoticeDB.setPrtSeq(mCertifyNo); //单证管理表中主键CertifyNo为打印管理表中的打印流水号
        ...
        mLCPENoticeSchema.setPrintFlag("2"); //体检通知书已回收
