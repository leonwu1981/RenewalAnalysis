...
public class PEdorAppPrintRReportAfterInitService implements AfterInitService{
  ...
  /** 错误处理类，每个需要错误处理的类中都放置该类 */
   ...
   /** 保全核保主表 */
   private LPAppUWMasterMainSchema mLPAppUWMasterMainSchema = new LPAppUWMasterMainSchema();
   /** 打印管理表 */
   ...
   public PEdorAppPrintRReportAfterInitService() {
  }
...
//   mLCPolSchema.setSchema(tLCPolDB) ;
   ...
   //tLPAppUWMasterMainDB.setPolNo(mPolNo) ;
   ...
   tLOPRTManagerDB.setOtherNoType( PrintManagerBL.ONT_EDORACCEPT );//保单号
...
//	}
  ...
  private boolean preparePrintRReport()
  {
	...
	if(mPatchFlag==false)//不是补打，则更新数据状态
	{
	  ...
	  mLPAppUWMasterMainSchema.setReportFlag("2") ;//打印面见通知书
	}
   ...
   private boolean prepareAutoSysCertSendOut()
   {
	 ...
	 if(mAutoSysCertSendOutFlag == true)//需要自动发放，则更新数据状态
	 {
	  ...
	  tLZSysCertifyschema.setTakeBackNo(PubFun1.CreateMaxNo("TAKEBACKNO", PubFun.getNoLimit(mManageCom.substring(1))));
	  tLZSysCertifyschema.setSendNo(PubFun1.CreateMaxNo("TAKEBACKNO", PubFun.getNoLimit(mManageCom.substring(1))));
...
//	  String tsql = "select max(serialno) from lPAppRReport where EdorAcceptNo = '"+mEdorAcceptNo+"'";
//	  ExeSQL tExeSQL = new ExeSQL();
//	  String tSerialNo = tExeSQL.getOneValue(tsql);

	  //tLPAppRReportDB.setSerialNo(tSerialNo);
	  //tLPAppRReportDB.setPolNo(mPolNo) ;
          tLPAppRReportDB.setEdorAcceptNo(mEdorAcceptNo);
          tLPAppRReportDB.setPrtSeq(mPrtSeq);
	  tLPAppRReportSet = tLPAppRReportDB.query();
	  if(tLPAppRReportSet == null || tLPAppRReportSet.size() !=1)
	  {
		// @@错误处理
		CError tError = new CError();
		tError.moduleName = "PEdorUWAutoHealthAfterInitService";
		tError.functionName = "prepareTransferData";
		tError.errorMessage = "保全生调表LPAppRReport查询失败!";
		this.mErrors.addOneError(tError) ;
		return false;
	  }

      tLPAppRReportSchema = tLPAppRReportSet.get(1) ;

	  //mTransferData.setNameAndValue("InsuredNo",tLPAppRReportSchema.getInsuredNo());
	  //mTransferData.setNameAndValue("InsuredName",tLPAppRReportSchema.getInsuredName()) ;
	  //mTransferData.setNameAndValue("Code","24") ;下面已准备
	  mTransferData.setNameAndValue("RROperator",tLPAppRReportSchema.getOperator());
	  mTransferData.setNameAndValue("HandleDate",tLPAppRReportSchema.getMakeDate());
	  //mTransferData.setNameAndValue("SerialNo",tLPAppRReportSchema.getSerialNo() ) ;
