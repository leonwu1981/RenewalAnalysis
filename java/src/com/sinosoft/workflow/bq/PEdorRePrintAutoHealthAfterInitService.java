...
public class PEdorRePrintAutoHealthAfterInitService implements AfterInitService {
  ...
  /** 错误处理类，每个需要错误处理的类中都放置该类 */
	...
	public PEdorRePrintAutoHealthAfterInitService() {
  }
  ...
  private boolean checkData()
  {
	...
	if(tLOPRTManagerSet.size() != 1)
	{
	 ...
	 if(!mLOPRTManagerSchema.getCode().equals("23"))
	 {
	  CError tError = new CError();
	  tError.moduleName = "PEdorRePrintAutoHealthAfterInitService";
	  tError.functionName = "checkData";
	 ...
	 if(mLOPRTManagerSchema.getStateFlag() == null || !mLOPRTManagerSchema.getStateFlag().equals("1"))
	 {
	  ...
	  tError.moduleName = "PEdorRePrintAutoHealthAfterInitService";
	  tError.functionName = "checkData";
	  tError.errorMessage = "体检通知书"+mLOPRTManagerSchema.getPrtSeq()+"标识未处于已打印,未回收状态,不容许进行补打!";
	  this.mErrors .addOneError(tError) ;
	  return false;
	 }
