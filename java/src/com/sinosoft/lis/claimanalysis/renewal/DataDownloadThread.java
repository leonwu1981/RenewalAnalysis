...
//			}
			...
			int people = 1;
			if("无名单".equals(insuredInfo.getCustomerName())){
				people = Integer.parseInt(exeSQL.getOneValue("select INSUREDPEOPLES from insured_view where contno='"+insuredInfo.getContno()+"'"));
			}
			...
			insuredInfo.setExposure(x*people/y);
...
//				int day1 = FormatConverter.getInt( date1[2] );
			...
			insuredInfo.setExposure2(x*people/days_a_year);
...
//			misInfo.append( key  + "," );
     ...
     */
    public String[] getPayRateTrue(String tGrpContNo,String tRiskCode,String tContPlanCode,String tDutyCode,String tGetDutyCode,String tGetLimit,String tGetRate){
    //得到起付线、赔付比例,意外赔付比例,日药费限额、日费用限额、日床位费限额、日检查费限额、
...
//	        }
    ...
    public String[] getPayRate(String tGrpContNo,String tRiskCode,String tContPlanCode,String tDutyCode,String tGetDutyCode,String tGetLimit,String tGetRate){
		String payRateKey = tGrpContNo+"^"+tRiskCode+"^"+tContPlanCode+"^"+tDutyCode+"^"+tGetDutyCode+"^"+tGetLimit+"^"+tGetRate;
		String GetRate[] = {"0","0","0","0","0","0","0"} ;
		if(payRateHm.containsKey(payRateKey)){
			
		}else{
			GetRate = getPayRateTrue(tGrpContNo,tRiskCode,tContPlanCode,tDutyCode,tGetDutyCode,tGetLimit,tGetRate);
			payRateHm.put(payRateKey, GetRate);
		}
		return (String[]) payRateHm.get(payRateKey);
