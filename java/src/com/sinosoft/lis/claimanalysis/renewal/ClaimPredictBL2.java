...
//		Date date = new Date();
	...
	public boolean exportPredictiveClaim(){
		...
		IbnrFactorCalculator2 c = new IbnrFactorCalculator2(file_path, jasperPath, id, "", startYear, startMonth, endYear, endMonth);
//		if( !c.calIbnrFactors()){
////			return false;
//		}
...
//				return false;
		...
		if(files.size()>0){
			FileProcessor.mergeCsvToXls(files, sheetNames, file_path + "predictclaim_ALL.xls");
		}
		return true;
	}
...
//				}
				...
				String key = incurred + "#" + grpContNo + "#" + relation + "#" + riskCode + "#" + dutyCode;
				Object o = realPayMapNew.get(key);
				if( o == null ){
					realPayMapNew.put(key, new Double(realPay));
				} else {
					Double value = (Double)o;
					realPayMapNew.put(key, new Double( value.doubleValue() + realPay ));
				}

			}
			
			//3.exposure
			HashMap matchMapNew = new HashMap();//放身份、险种、责任的exposure
			HashMap earnedMapNew = new HashMap();//放身份、险种、责任的earnedprem
			ArrayList alGrpcontno = new ArrayList();
			ArrayList alRelation = new ArrayList();
			ArrayList alRiskcode = new ArrayList();
			ArrayList alDutyCode = new ArrayList();
			HashMap hmDuty = new HashMap();
...
//				String planCode = row[19].replaceAll("_", "");//弹性福利计划的planCode有_，会导致后面的计算有问题
				...
				String dutyName = row[49];
				String relation = row[16];
				double prem = Double.parseDouble( row[24] );
				
				hmDuty.put(dutyCode, dutyName);
				
				if(!alGrpcontno.contains(grpContNo)){
					alGrpcontno.add(grpContNo);
				}
				if(!alRelation.contains(relation)){
					alRelation.add(relation);
				}
				if(!alRiskcode.contains(riskCode)){
					alRiskcode.add(riskCode);
				}
				if(!alDutyCode.contains(dutyCode)){
					alDutyCode.add(dutyCode);
				}
...
//					d1 = d1.after(cdate)?d1:cdate;
					...
					if(earnedprem>0){
						...
						String key = incurred + "#" + grpContNo + "#" + relation + "#" + riskCode + "#" + dutyCode;
						Object o = earnedMapNew.get(key);
						if( o == null ){
							earnedMapNew.put(key, new Double(earnedprem));
						} else {
							Double value = (Double)o;
							earnedMapNew.put(key, new Double( value.doubleValue() + earnedprem ));
						}
					}
...
//					}
					...
					if(exposure>0){
						String incurred = (m1<10)?(y1+"/0"+m1):(y1+"/"+m1);
						String key = incurred + "#" + grpContNo + "#" + relation + "#" + riskCode + "#" + dutyCode;
						Object o = matchMapNew.get(key);
						if( o == null ){
							matchMapNew.put(key, new Double(exposure));
						} else {
							Double value = (Double)o;
							matchMapNew.put(key, new Double( value.doubleValue() + exposure ));
						}
...
//						}
			...
			Collections.sort(alGrpcontno);
			Collections.sort(alRelation);
			Collections.sort(alRiskcode);
			Collections.sort(alDutyCode);
...
//			Collections.sort(relationList);
			...
			String head = "发生月份incurred month,保单号,身份,险种,责任代码,责任名称,曝露数exposure,已赚保费earned prem,实际发生金额Paid Claim";
...
//				double sumEarnedprem = 0;
				for(int i=(startYear*12+startMonth); i<=(endYear*12+endMonth); i++){
					...
					for (int j = 0; j < alGrpcontno.size(); j++) {
						String grpContNo = (String) alGrpcontno.get(j);
						for (int k = 0; k < alRelation.size(); k++) {
							String relation = (String) alRelation.get(k);
							for (int l = 0; l < alRiskcode.size(); l++) {
								String riskCode = (String) alRiskcode.get(l);
								for (int n = 0; n < alDutyCode.size(); n++) {
									String dutyCode = (String) alDutyCode.get(n);
									
									String key = incurred + "#" + grpContNo + "#" + relation + "#" + riskCode + "#" + dutyCode;
									
									if(matchMapNew.containsKey(key)||earnedMapNew.containsKey(key)||realPayMapNew.containsKey(key)){
										sb.append(incurred).append(",").append(grpContNo).append(",").append(relation).append(",").append(riskCode).append(",").append(dutyCode).append(",").append(hmDuty.get(dutyCode)).append(",");
										if(matchMapNew.containsKey(key)){
											sb.append(matchMapNew.get(key)).append(",");
										}else{
											sb.append("0").append(",");
										}
										if(earnedMapNew.containsKey(key)){
											sb.append(earnedMapNew.get(key)).append(",");
										}else{
											sb.append("0").append(",");
										}
										if(realPayMapNew.containsKey(key)){
											Double d = (Double) realPayMapNew.get(key);
											double realPay = Arith.round(d.doubleValue(), 2);
											sb.append(realPay);
										}else{
											sb.append("0");
										}
										
//										int nn =  endYear*12 + endMonth - y*12 - m;
//										double cf;
//										if(nn>=ibnrfactors.length){
//											cf = 1;
//										} else {
//											cf = ibnrfactors[nn];
//										}
										sb.append("\r\n");
										if(sb.length()>=1024*1024*5){
