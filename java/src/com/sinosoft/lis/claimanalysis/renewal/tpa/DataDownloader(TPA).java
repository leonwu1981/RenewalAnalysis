...
public class DataDownloader {
	...
	public void downloadData(String filePath, String grpContNo, String startDate, String endDate, String manageCom, String riskCode,String orgs){
		...
		do {
			if( !getContBatchInfo(grpContNo, startDate, endDate, manageCom, riskCode,orgs) ){
				continue;
			}
			boolean flag = true;
...
//					Thread thread = new Thread(new DataDownloadThread(threadNum, minContNo, maxContNo, file_path+"data/", grpContNo, startDate, endDate, manageCom, riskCode, lmDutyGetRelaHt)); 
					Thread thread = new Thread(new DataDownloadThread(threadNum, minContNo, maxContNo, filePath, grpContNo, startDate, endDate, manageCom, riskCode, lmDutyGetRelaHt,orgs)); 
					thread.start(); 
					flag = false;
				} else {
					try {
		...
		// head
		String claim_head = "分公司代码,团体保单号,保单生效日,保单终止日,被保险人客户号,出生年月,性别,被保险人生效日,被保险人终止日,在职或退休,与被保险人的关系,计划编码,计划名称,被保险人赔案号,出险日期,住院结束日期,住院天数,就医医院代码,就医医院名称,医院等级,索赔原因,申请赔付日期,费用项目代码,费用项目名称,费用金额,自费金额,部分自付金额,医保支付金额,索赔项目代码,索赔项目名称,实际赔付金额,赔付日期,险种代码,出险时间段,理赔时间段,理赔延迟时间,年份,险种名,门诊日限额,检查费日限额,保额,赔付比例,是否TPA标记,是否TPA赔付,行业,姓名,GEB客户,部门编码\r\n";
		FileProcessor.outputFile(claim_head, file_path, "claim_0", "csv");
		String insured_head = "分公司代码,团体保单号,保单生效日,保单终止日,是否续保,投保单位,投保单位部门,单位性质,行业类别,主营业务,被保险人客户号,被保险人姓名,被保险人生效日,被保险人终止日,被保险人职业等级,在职或退休,与被保险人的关系,出生年月,性别,计划编码,计划名称,险种,责任,保额,保费,起付线,日药费限额,日费用限额,日床位费限额,日检查费限额,赔付比例,意外赔付比例,服务机构,年龄,年龄段,曝光数,曝光数2,费用金额,自费金额,部分自付金额,医保支付金额,实际赔付金额,门诊就诊次数,就诊次数,就诊人数,住院天数,保全类型,年份,险种名,责任名,缴至日期,行业,GEB客户,部门编码,是否TPA标记,是否TPA赔付\r\n";
