...
//	Hashtable lcHt = new Hashtable();
	...
	DataDownloadThread( int threadNum, String minContNo, String maxContNo, String path, String grpContNo, String startDate, String endDate, String manageCom, String riskCode, HashMap lmDutyGetRelaMap, String orgs ) {
...
//		sb.append(" and b.insuredno='100390947488' ");
...
//				int day1 = FormatConverter.getInt( date1[2] );
		...
		for (int i = 0; i < list.size(); i++) {
			InsuredInfo insuredInfo = (InsuredInfo) list.get(i);
			
			insuredInfo.setTpaflag((String)hmTPA.get(insuredInfo.getGrpContNo()+"^"+insuredInfo.getContPlanCode()));
			if("是TPA计划".equals(insuredInfo.getTpaflag())&&insuredInfo.getInnetwork()==null){
				insuredInfo.setInnetwork("TPA赔付");
				
				InsuredInfo tmpInsuredInfo = (InsuredInfo) insuredInfo.clone();
				tmpInsuredInfo.setAmnt(0);
				tmpInsuredInfo.setPrem(0);
				tmpInsuredInfo.setExposure(0);
				tmpInsuredInfo.setExposure2(0);
				tmpInsuredInfo.setInnetwork("非TPA赔付");
				list.add(tmpInsuredInfo);
			}
...
//		sql.append("    and b.contno in ('600322527588')                         ");
...
//						"b.mainfeeno," +
       			...
       			+ " ,b.hospitalname "//医院名称
       			+ " from llcasereceipt a,llfeemain b where a.caseno='"+tCaseNo+"'  "
       			+ " and a.caseno=b.caseno and a.mainfeeno=b.mainfeeno and a.fee>0 "
       			+ " and a.feeitemcode in(select feecode from lmdutygetfeerela where getdutycode='"+tGetDutyCode+"')  ";
//				tmpDataTable = mDataAccess.executeDataTable(tmpSql);
			...
			do{
				...
				//del by wuk。从下个月（2013-2）开始的续保报表需要将理赔文件中的出险日期修改为赔案上的出险日期(llcase.ACCIDENTDATE)，麻烦帮忙处理
				/**
				...
				if(claimInfo.getRiskCode().equals("NIK03")||claimInfo.getRiskCode().equals("NIK07")||
					...
					//42赔付比例
					try{
						claimInfo.setPayRatio(FormatConverter.getDouble(tPayRate[1]));
					}catch(Exception e){
						e.printStackTrace();
					}
					//43是否TPA标记
...
//           				claimInfo.setAccidentDate( FormatConverter.getDate(tmpSSRS.GetText(i, 3)) );
...
//				ClaimInfo clm = (ClaimInfo)this.clone(claimInfo);
	...
	public Hashtable queryClmBenAnnual( Hashtable clmPerVisHt ) {
		...
		for(Iterator i = clmPerVisHt.values().iterator(); i.hasNext(); ){
			...
			key.append("#");
			key.append(claimInfo.getInnetwork());//是否TPA赔付
...
//				ClaimInfo clm = (ClaimInfo)this.clone(claimInfo);
	...
	public List matchCertClm( List insList, Hashtable claimAnnual ) {
		...
		for(int i = 0;i<insList.size(); i++ ) {
			InsuredInfo insuredInfo = (InsuredInfo)insList.get(i);
			String dutyCode = insuredInfo.getDutyCode();
			if(dutyCode==null){
				continue;
			}
			...
			for(int j=0;j<getDutyCodes.length;j++){
				...
				key.append("#");
				key.append( insuredInfo.getInnetwork() );//赔付责任
			...
			if("是TPA计划".equals(insuredInfo.getTpaflag())&&"非TPA赔付".equals(insuredInfo.getInnetwork())){
				if(insuredInfo.getExpenseAmnt()<0.01&&insuredInfo.getOwnExpenseAmnt()<0.01
						&&insuredInfo.getPartExpenseAmnt()<0.01&&insuredInfo.getSsExpenseAmnt()<0.01
						&&insuredInfo.getRealPay()<0.01){
					insList.remove(insuredInfo);
					i--;
				}
			}
		}
...
//	        }
    ...
    public static void main(String[] args) {
    	String[] tPayRate = new DataDownloadThread().getPayRate( "83000029500", "NIK12", "B", "602001", "602202", null, null );
